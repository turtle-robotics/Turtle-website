      ...
      <div className="max-w-6xl mx-auto px-4 py-10">
        ...
        <div className="mb-12 text-center">
          ...
          <div className="flex flex-wrap items-center justify-center gap-3">
            <span className="text-xs font-light text-gradient-foreground bg-accent px-3 py-1 rounded-full">{project.category}</span>
            {project.tags?.slice(0,4).map((tag, i) => (
              <span key={i} className="text-xs font-light text-text bg-surface border border-border px-3 py-1 rounded-full">{tag}</span>
            ))}
          </div>
          <div className="project-lead mt-6">
            <p className="text-base md:text-lg text-text">
              Lead: {project.lead} <span className="text-accent ml-2">({project.leadEmail || project.email})</span>
            </p>
          </div>
        </div>
        ...
        {slides && slides.length > 0 && (
          <div className="project-section mb-12">
            <div className="rounded-2xl overflow-hidden border border-border glass-card" style={{ height: '360px' }}>
        ...
        {/* Project Updates */}
        <div className="project-section mb-12">
          <h2 className="text-2xl font-light text-text mb-6 text-center">Project Updates</h2>
          {project.updates && project.updates.length > 0 ? (
            <div className="space-y-6">
              {project.updates.map((update, index) => (
                <div key={index} className="glass-card rounded-2xl p-6">
                  <div className="flex items-center justify-between mb-2">
                    <span className="text-sm text-muted">{formatDate(update.date)}</span>
                    <span className="text-xs font-light text-gradient-foreground bg-accent px-3 py-1 rounded-full">Update</span>
                  </div>
                  <h3 className="text-lg font-light text-text mb-2">{update.title}</h3>
            ...
            <div className="glass-card rounded-2xl p-6 text-center">
              <p className="text-muted mb-3">No updates have been posted yet.</p>
              <a
        ...
        <div className="mt-12 pt-8 border-t border-border">
          <Link 
            to="/apply" 
            className="inline-block bg-gradient-turtle text-gradient-foreground px-8 py-3 rounded-lg font-medium hover:scale-105 transition-all duration-300"
          >
