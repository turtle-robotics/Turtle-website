 ...
 *
 * Displays all TURTLE projects with search and AI-powered recommendation features.
 * Users can search by keywords or use AI to match projects to their interests.
 * Clicking a project card navigates to detailed project page.
 */
...
const Projects = () => {
  ...
  const [aiResult, setAiResult] = useState(null); // AI recommendation results
  const [isLoadingAI, setIsLoadingAI] = useState(false);
  const [aiAutoMode, setAiAutoMode] = useState(false); // Auto-trigger AI when no matches
   ...
   * Calls AI service or falls back to local keyword matching
   */
  ...
  /**
   * Auto-trigger AI mode when no direct matches found
   * Waits 500ms after typing stops, only triggers if:
   * - Query is at least 3 characters
   * - No projects match the search term
   */
  useEffect(() => {
            ...
            <div className="relative w-full max-w-3xl px-2">
              <div className="glass-card relative mx-auto flex items-center justify-center gap-3 rounded-2xl px-4 py-3 border-2 border-accent shadow-lg backdrop-blur">
                <span
                  ...
                  style={{
                    filter:
                      "drop-shadow(0 0 1px var(--color-gray-500)) drop-shadow(0 0 2px var(--color-gray-500))",
                  }}
                >
                  ⚡
                </span>
                ...
                <span
                  ...
                  style={{
                    filter:
                      "drop-shadow(0 0 1px var(--color-gray-500)) drop-shadow(0 1px 3px var(--color-gray-500))",
                  }}
                >
                  ✨
                </span>
        ...
        <div className="max-w-7xl mx-auto">
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
            {filteredProjects.map((project, index) => (
              <div
                key={index}
                className="group glass-card rounded-2xl overflow-hidden transition-shadow duration-300 shadow-soft cursor-pointer hover:shadow-accent"
              ...
              >
                ...
                <div className="p-6 flex flex-col h-full">
                  <h3 className="text-xl font-light text-text mb-2 group-hover:text-accent transition-colors duration-300">
                    {project.title}
                  </h3>
                  <p className="text-sm font-regular text-accent mb-3">
                    {project.subtitle}
                  </p>
                  <p className="text-sm font-regular text-muted leading-relaxed mb-4 line-clamp-3">
                    {project.description}
                  </p>
