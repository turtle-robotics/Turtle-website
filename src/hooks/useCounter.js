/**
 * useCounter Hook
 *
 * Custom hook for animated number counters with scroll trigger.
 * Uses GSAP to animate from 0 to target value when element enters viewport.
 *
 * @param {number} targetValue - Final count to reach
 * @param {number} duration - Animation duration in seconds (default: 2)
 * @returns {{count: number, counterRef: React.RefObject}} Current count and ref for scroll trigger
 *
 * @example
 * const { count, counterRef } = useCounter(250, 2.5);
 * return <div ref={counterRef}>{count}+</div>;
 */
import { useEffect, useRef, useState } from "react";
import { gsap } from "gsap";

export const useCounter = (targetValue, duration = 2) => {
  const [count, setCount] = useState(0);
  const counterRef = useRef(null);

  useEffect(() => {
    const element = counterRef.current;
    if (!element) return;

    gsap.to(
      {},
      {
        duration,
        onUpdate: function () {
          const progress = this.progress();
          const currentCount = Math.floor(targetValue * progress);
          setCount(currentCount);
        },
        scrollTrigger: {
          trigger: element,
          start: "top 80%",
          end: "bottom 20%",
          toggleActions: "play none none reverse",
        },
      },
    );
  }, [targetValue, duration]);

  return { count, counterRef };
};
